...
def total_list_songs_popularity():
    ...
    cur.execute('CREATE TABLE Spotipy (artist_id INTEGER, song VARCHAR NOT NULL PRIMARY KEY UNIQUE, popularity INTEGER)')
    for item in popularity_list[0:25]:
        cur.execute('INSERT INTO Spotipy (artist_id, song, popularity) VALUES (?,?,?)''', (item[0], item[1], item[2]))
    conn.commit()

    for item in popularity_list[25:50]:
        cur.execute('INSERT INTO Spotipy (artist_id, song, popularity) VALUES (?,?,?)''', (item[0], item[1], item[2]))
    conn.commit()

    for item in popularity_list[50:75]:
        cur.execute('INSERT INTO Spotipy (artist_id, song, popularity) VALUES (?,?,?)''', (item[0], item[1], item[2]))
    conn.commit()

    for item in popularity_list[75:100]:
        cur.execute('INSERT INTO Spotipy (artist_id, song, popularity) VALUES (?,?,?)''', (item[0], item[1], item[2]))
    conn.commit()
    ...
    cur.execute(""" SELECT Spotipy.popularity, Spotipy2.duration_ms
                FROM Spotipy
                JOIN Spotipy2
                ON Spotipy.artist_id = Spotipy2.artist_id
                """)
    res = cur.fetchall()
    conn.commit()
...
def total_list_songs_duration_ms():
    ...
    cur.execute('DROP TABLE IF EXISTS Spotipy2')
    cur.execute('CREATE TABLE Spotipy2 (artist_id INTEGER, song VARCHAR NOT NULL PRIMARY KEY UNIQUE, duration_ms INTEGER)')
    for item in duration_ms_list:
        cur.execute('INSERT INTO Spotipy2 (artist_id, song, duration_ms) VALUES (?,?,?)''', (item[0], item[1], item[2]))
    conn.commit()
